<?php
    $path = '/var/www/html/seguimiento/export/Reporte.xls'; 
    $array_data = json_decode($_POST['arreglo']);
    $fp = fopen($path, 'w');
    $b = '\@';
    $array_headers = json_decode($_POST['titulos']);
    fputs($fp, '<table><tr bgcolor="#f0f0f0">');
    array_walk_recursive($array_headers[0], 'ExportExcelHeaders', $fp);
    fputs($fp, "</tr>");
    fputs($fp, '<tr bgcolor="#f0f0f0">');
    array_walk_recursive($array_headers[1], 'ExportExcelHeaders', $fp);
    fputs($fp, "</tr>");
